...
public class FrontController {
    ...
    private String handleDocument(
        ...
        try {
            ...
            // ===== 3. IMPOSTA SECTIONTYPE =====
            // Prima prova dalla sezione parent (caso normale)
            // Se la sezione parent non esiste (es. sezione non migrata in locale),
            // carica il SectionType direttamente dall'id_type del documento
            if (parentSection != null) {
                config.setActualSection(parentSection);
                document.setSection(parentSection);
            } else {
                // Fallback: crea Section fittizia con solo il SectionType dal documento
                if (document.getIdType() != null && !document.getIdType().isEmpty()) {
                    try {
                        ...
                        if (sectionType != null) {
                            ...
                            log.debug("SectionType caricato da id_type documento: {}", sectionType.getPage4());
                        }
                    } catch (Exception e) {
            ...
            // ===== 4. CARICA EXTRATAG (contenuti correlati) =====
