 ...
 */
 ...
 (function ( $ ) {
    ...
    // week names
    var weekNames = ["domenica", "lunedì", "martedì", "mercoledì", "giovedì", "venerdì", "sabato"];
      ...
      return weeks;
		...
		*/
		var draw = function() {
          ...
          // create header label
          var headerGroup = $("<div id='dncalendar-header' class='dncalendar-header mb-5'></div>");
          ...
          var bodyGroup = $("<div id='dncalendar-body' class='dncalendar-body  d-none d-sm-none d-md-block'></div>");
          var tableGroup = $("<table class='table table-striped-columns table-responsive'></table>");
          ...
          for (var i = 0; i < nDates; i++) {
            ...
            for (var j = 0; j < nDate; j++) {
              ...
              if (defDate != null && defDate.getFullYear() == year && (defDate.getMonth() + 1) == month && defDate.getDate() == date) {
                ...
                colDateDataAttr = defaultDateTitle;
              }
              ...
              if (typeof settings.notes !== 'undefined') {
                if (dateIsNotes(new Date(year, month - 1, date))) {
                  colDateClass += " note ";
				  //console.log("evento", getdateIsNotes(new Date(year, month - 1, date)));
				  eventsDateDataAttr += getdateIsNotes(new Date(year, month - 1, date));
                }
              }

              var colDate = "<td id='calendarClick' class='"+ colDateClass +" "+ ((showCalendarClick == true) ? 'calendarClick' : '') +"' data-date='"+ date +"' data-month='"+ month +"' data-year='"+ year +"'><div class='entry' data-title='"+ colDateDataAttr +"'>"+ date +"<br>"+eventsDateDataAttr+"</div></td>";
          ...
          if (settings.showNotes) {
                ...
                if (notesLength > 0) {
                      notesGroup = $("<ul class='dncalendar-note-list d-block d-sm-block d-md-none'></ul>");

                      for (var i = 0; i < notesLength; i++) {
						  var date = notes[i].date;
						  var link = notes[i].link;
						  var img = notes[i].img;
						  ...
						  if (noteListLength > 0) {
							...
							for (var j = 0; j < noteListLength; j++) {
							  list += "<div class='card card-teaser rounded shadow mb-4'>" + "<div class='card-body'>" + "<a href='" + link[j] + "'>" + "<h3 class='card-title h5'>" + "<span class='text-decoration-none me-2'>" + date + "</span>" + "<img class='avatar size-lg me-2' src='" + img[j] + "'>"  + noteList[j] + "</h3>" + "</a>";
							  if (j < noteListLength - 1) {
								list += ", ";
							  }
							}
						  }
	...
	var getdateIsNotes = function(date) {
		let eventi="";
          var notesLength = settings.notes.length;
          for (var i = 0; i < notesLength; i++) {
                var dateNote = settings.notes[i].date.split('-');
                ...
                if ( nDate.getFullYear() == date.getFullYear() && nDate.getMonth() == date.getMonth() && nDate.getDate() == date.getDate() ) {
                      eventi += "<a class='badge bg-primary eventoCalendario ' href='"+settings.notes[i].link+"' title='"+settings.notes[i].title+"'>"+settings.notes[i].note+"</a>";
                }
          }
    ...
    var getNotesThisMonth = function() {
          ...
          for (var i = 0; i < notesLength; i++) {
                ...
                if (nDate.getFullYear() == currDate.getFullYear() && nDate.getMonth() == currDate.getMonth()) {
					  ...
					  temp['titles'] = settings.notes[i].title;
					  temp['link'] = settings.notes[i].link
					  temp['img'] = settings.notes[i].img;
		...
		var triggerAction = function() {
		    ...
		    $('body').on('click', '#' + self.attr('id') + ' #calendarClick', function(){
				//console.log('#' + self.attr('id') + ' #calendarClick');
  ...
  $.fn.dnCalendar.defaults = { 
        ...
        showNotes: false,
        startWeek: 'Lunedì',
        dayClick: function(date, view) {alert(view);}
